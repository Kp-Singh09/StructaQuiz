...
function Draggable({ id, children }) {
  ...
  return <div ref={setNodeRef} style={style} {...listeners} {...attributes} className="bg-white py-1 px-3 rounded-md shadow border cursor-grab">{children}</div>;
}

function Droppable({ id, children, isFilled }) {
  const { setNodeRef } = useDroppable({ id });
  const baseClasses = "p-2 min-w-[100px] min-h-[40px] border-2 border-dashed rounded-md flex items-center justify-center";
  const filledClasses = "border-green-400 bg-green-50";
  const emptyClasses = "border-gray-300 bg-gray-100";
  ...
  return (
    <DndContext onDragEnd={handleDragEnd}>
      ...
      {question.image && <img src={question.image} alt="Question visual" className="w-full h-40 object-cover rounded-md mb-4" />}
        <h3 className="text-xl font-bold mb-4 text-gray-800">Fill in the Blanks</h3>
        
        <div className="text-lg flex flex-wrap items-center gap-2 mb-6">
        ...
        <div className="p-4 bg-gray-50 border rounded-lg min-h-[60px]">
          <h4 className="font-semibold mb-2">Drag these words:</h4>
