  ...
  return (
    <>
      ...
      <div className="fixed inset-0 -z-10">
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 0.5, scale: 1 }} transition={{ duration: 2 }} className="absolute left-[-10vw] top-[-10vh] w-[40vw] h-[40vw] bg-blue-700/30 rounded-full blur-3xl" />
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 0.4, scale: 1 }} transition={{ duration: 2, delay: 0.5 }} className="absolute right-[-10vw] top-[20vh] w-[30vw] h-[30vw] bg-purple-500/20 rounded-full blur-2xl" />
      </div>
      ...
      <motion.main
        ...
        className="min-h-screen pt-28 bg-[#0A0F1F] text-white relative bg-[length:80px_80px] bg-[linear-gradient(transparent_79px,#232733_80px),linear-gradient(90deg,transparent_79px,#232733_80px)]"
      >
        ...
        <section className="text-center pt-16 md:pt-24 px-4">
          ...
          >
            <h1 className="text-4xl md:text-6xl font-bold leading-tight text-slate-100 drop-shadow-xl">
              Build Interactive Quizzes That <span className="text-blue-400">Engage</span> and <span className="text-purple-400">Impress</span>
            </h1>
        ...
        <section className="max-w-6xl mx-auto mt-24 md:mt-32 px-4 sm:px-6">
            <h2 className="text-3xl font-bold mb-12 text-center text-slate-200">Everything You Need to Build Better Quizzes</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {features.map((feature, index) => (
                    <motion.div
                        ...
                        viewport={{ once: true }}
                        className="bg-slate-800/40 p-6 rounded-2xl border border-slate-700/80"
                    >
                        <div className="flex items-center justify-center w-12 h-12 rounded-full bg-blue-700/30 shadow-lg text-3xl mb-4">{feature.icon}</div>
        ...
        <section className="max-w-3xl mx-auto mt-24 md:mt-32 mb-20 px-4 sm:px-6">
          <h2 className="text-3xl font-bold mb-8 text-center text-blue-300">Frequently Asked Questions</h2>
          <div className="space-y-4">
            {faqData.map((item, idx) => (
              <div key={idx}>
                <button
                  onClick={() => toggleFAQ(idx)}
                  className={`w-full bg-[#0f172a] text-left px-5 py-4 rounded-lg border border-blue-400/40 text-lg font-semibold focus:outline-none flex justify-between items-center transition-all duration-300 ${openIndex === idx ? 'ring-2 ring-blue-400 bg-slate-800/60' : 'hover:bg-slate-800/50'}`}
                >
                  <span>{item.question}</span><span className={`ml-4 text-blue-300 transition-transform duration-300 ${openIndex === idx ? 'rotate-180' : ''}`}>▼</span>
                </button>
                <AnimatePresence>
                {openIndex === idx && (
                  <motion.div
                  ...
                  >
                    <div className="px-5 py-3 bg-slate-800/50 text-slate-300 border-l-4 border-blue-400/60 rounded-b-lg">
                      {item.answer}
                    </div>
                  </motion.div>
                )}
